...
VIEWER_ANON_REPLACE_STRING_KEYS = {
    "caption",
    "content",
    "description",
    "empty",
...
def _viewer_replace_known_labels(value: str, state: dict[str, Any]) -> str:
    result = value
    for original, masked in state.get("label_replacements") or ():
...
def _viewer_anonymize_value(
    ...
    if isinstance(value, str):
        ...
        if key_name in VIEWER_ANON_REPLACE_STRING_KEYS:
...
def _viewer_anonymize_context(context: dict[str, Any]) -> dict[str, Any]:
    ...
    state["label_replacements"] = _viewer_known_label_replacements(state)
...
def _member_payout_fee_lines_from_rules(
    ...
    return [
        {
            "snapshot_id": int(rule.get("snapshot_id") or 0),
...
def _my_alliance_payout_context(
    ...
    if selected_alliance is None:
        return {
            ...
            "summary": {
                "total_text": "0",
                "total_cash_text": "0원",
                "settled_total_text": "0",
                "settled_total_cash_text": "0원",
                "fee_text": "0",
                "fee_cash_text": "0원",
                "unsettled_text": "0",
    ...
    settled_total_amount_sum = Decimal("0")
    settled_total_cash_sum = Decimal("0")
    ...
    for event in events:
        for alliance_id in selected_alliance_ids:
            ...
            if status == "paid":
                settled_count += 1
                settled_total_amount_sum += total_amount
                settled_total_cash_sum += _cash_from_adena(total_amount, adena_rate)
    ...
    return {
        ...
        "summary": {
            ...
            "settled_total_text": _money_text(settled_total_amount_sum),
            "settled_total_cash_text": _cash_text(settled_total_cash_sum),
