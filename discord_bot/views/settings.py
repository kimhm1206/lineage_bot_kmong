...
class SettingsMenuView(discord.ui.View):
    ...
    @discord.ui.select(
        ...
        options=[
            ...
            discord.SelectOption(
                ...
                description="출석 로그를 보낼 채팅채널을 선택합니다.",
            ),
            discord.SelectOption(
                label="타이머 설정",
                value="timer",
    ...
    async def settings_select(
        ...
        if selected == "log_channel_id":
            view = LogChannelSettingView(self.bot, self.guild_id)
            await interaction.response.edit_message(
                content=_text_channel_prompt(view.page, view.page_count),
                view=view,
            )
            return
...
class LogChannelSettingView(discord.ui.View):
    def __init__(self, bot: discord.Bot, guild_id: int, page: int = 0):
        super().__init__(timeout=300)
        self.bot = bot
        self.guild_id = guild_id
        ...
        self.page_count = max(
            1,
            (len(self.text_channels) + _channels_per_page() - 1) // _channels_per_page(),
        )
        self.page = max(0, min(self.page, self.page_count - 1))
        self._build_components()
    ...
    async def previous_button(
        ...
        new_page = self.page - 1
        await interaction.response.edit_message(
            content=_text_channel_prompt(new_page, self.page_count),
            view=LogChannelSettingView(self.bot, self.guild_id, new_page),
        )
...
def _channels_per_page() -> int:
    return VOICE_CHANNELS_PER_SELECT * CHANNEL_SELECTS_PER_PAGE
...
def _text_channel_prompt(page: int, page_count: int) -> str:
    if page_count <= 1:
        return "로그를 보낼 채팅채널을 선택해주세요."
    return (
        f"로그를 보낼 채팅채널을 선택해주세요. "
        f"({page + 1}/{page_count}페이지)"
    )
