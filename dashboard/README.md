...
업무 메뉴는 담당 주체를 기준으로 `연합 운영`과 `내 혈맹 운영`을 분리합니다. 연합 경리는 드랍 등록과 혈맹별 1차 분배를, 혈맹 경리는 혈맹원별 2차 분배와 혈비를 담당합니다. 출석과 서버 운영은 공통 영역으로 둡니다.

```text
홈
연합 운영       # 연합 경리: 드랍 등록, 각혈 분배, 입찰, 연합 수수료
내 혈맹 운영    # 혈맹 경리: 혈맹원 분배, 혈비, 귀속, 내부 수수료
출석 · 통계     # 공통 조회와 출석 설정
서버 운영       # 오너 권한과 서버 공통 설정
```
