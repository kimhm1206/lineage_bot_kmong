"""Normalize settlement rounding treasury categories.
...
def upgrade() -> None:
    ...
    op.execute(
        ...
        WHERE entry.treasury_category_id = legacy.treasury_category_id
          AND legacy.category_name LIKE '분배 후 나머지[Drop#%]'
...
def downgrade() -> None:
    # The normalized category remains valid for the previous application
    # version, so an unsafe append-only ledger rewrite is intentionally avoided.
    pass
